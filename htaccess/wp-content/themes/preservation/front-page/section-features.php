<div class="front_page_section front_page_section_features<?php
	$preservation_scheme = preservation_get_theme_option( 'front_page_features_scheme' );
	if ( ! empty( $preservation_scheme ) && ! preservation_is_inherit( $preservation_scheme ) ) {
		echo ' scheme_' . esc_attr( $preservation_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( preservation_get_theme_option( 'front_page_features_paddings' ) );
	if ( preservation_get_theme_option( 'front_page_features_stack' ) ) {
		echo ' sc_stack_section_on';
	}
?>"
		<?php
		$preservation_css      = '';
		$preservation_bg_image = preservation_get_theme_option( 'front_page_features_bg_image' );
...
<?php
	// Add anchor
	$preservation_anchor_icon = preservation_get_theme_option( 'front_page_features_anchor_icon' );
	$preservation_anchor_text = preservation_get_theme_option( 'front_page_features_anchor_text' );
...
?>
	...
	<?php
	if ( preservation_get_theme_option( 'front_page_features_fullheight' ) ) {
	...
	?>
			...
			<?php
			$preservation_css      = '';
			$preservation_bg_mask  = preservation_get_theme_option( 'front_page_features_bg_mask' );
			$preservation_bg_color_type = preservation_get_theme_option( 'front_page_features_bg_color_type' );
			if ( 'custom' == $preservation_bg_color_type ) {
				$preservation_bg_color = preservation_get_theme_option( 'front_page_features_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_features_content_wrap content_wrap">
			<?php
			// Caption
			$preservation_caption = preservation_get_theme_option( 'front_page_features_caption' );
			if ( ! empty( $preservation_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_features_caption front_page_block_<?php echo ! empty( $preservation_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $preservation_caption, 'preservation_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$preservation_description = preservation_get_theme_option( 'front_page_features_description' );
			if ( ! empty( $preservation_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_features_description front_page_block_<?php echo ! empty( $preservation_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $preservation_description ), 'preservation_kses_content' ); ?></div>
				<?php
			}
			...
			<div class="front_page_section_output front_page_section_features_output">
				...
				if ( is_active_sidebar( 'front_page_features_widgets' ) ) {
					...
					if ( ! preservation_exists_trx_addons() ) {
						...
						preservation_customizer_need_widgets_message( 'front_page_features_caption', 'ThemeREX Addons - Services' );
					}
				}
				?>
			</div>
