...
if ( ! function_exists( 'preservation_storage_get' ) ) {
	/**
	 ...
	 */
	function preservation_storage_get( $var_name, $default = '' ) {
		global $PRESERVATION_STORAGE;
		return isset( $PRESERVATION_STORAGE[ $var_name ] ) ? $PRESERVATION_STORAGE[ $var_name ] : $default;
	}
}

if ( ! function_exists( 'preservation_storage_set' ) ) {
	/**
	 ...
	 */
	function preservation_storage_set( $var_name, $value ) {
		global $PRESERVATION_STORAGE;
		$PRESERVATION_STORAGE[ $var_name ] = $value;
	}
}

if ( ! function_exists( 'preservation_storage_empty' ) ) {
	/**
	 ...
	 */
	function preservation_storage_empty( $var_name, $key = '', $key2 = '' ) {
		global $PRESERVATION_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return empty( $PRESERVATION_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return empty( $PRESERVATION_STORAGE[ $var_name ][ $key ] );
		} else {
			return empty( $PRESERVATION_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'preservation_storage_isset' ) ) {
	/**
	 ...
	 */
	function preservation_storage_isset( $var_name, $key = '', $key2 = '' ) {
		global $PRESERVATION_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return isset( $PRESERVATION_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return isset( $PRESERVATION_STORAGE[ $var_name ][ $key ] );
		} else {
			return isset( $PRESERVATION_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'preservation_storage_unset' ) ) {
	/**
	 ...
	 */
	function preservation_storage_unset( $var_name, $key = '', $key2 = '' ) {
		global $PRESERVATION_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			unset( $PRESERVATION_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			unset( $PRESERVATION_STORAGE[ $var_name ][ $key ] );
		} else {
			unset( $PRESERVATION_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'preservation_storage_inc' ) ) {
	/**
	 ...
	 */
	function preservation_storage_inc( $var_name, $value = 1 ) {
		global $PRESERVATION_STORAGE;
		if ( empty( $PRESERVATION_STORAGE[ $var_name ] ) ) {
			$PRESERVATION_STORAGE[ $var_name ] = 0;
		}
		$PRESERVATION_STORAGE[ $var_name ] += $value;
	}
}

if ( ! function_exists( 'preservation_storage_concat' ) ) {
	/**
	 ...
	 */
	function preservation_storage_concat( $var_name, $value ) {
		global $PRESERVATION_STORAGE;
		if ( empty( $PRESERVATION_STORAGE[ $var_name ] ) ) {
...
if ( ! function_exists( 'preservation_storage_get_array' ) ) {
	/**
	 ...
	 */
	function preservation_storage_get_array( $var_name, $key, $key2 = '', $default = '' ) {
		global $PRESERVATION_STORAGE;
		if ( '' === $key2 ) {
			return ! empty( $var_name ) && '' !== $key && isset( $PRESERVATION_STORAGE[ $var_name ][ $key ] ) ? $PRESERVATION_STORAGE[ $var_name ][ $key ] : $default;
		} else {
			return ! empty( $var_name ) && '' !== $key && isset( $PRESERVATION_STORAGE[ $var_name ][ $key ][ $key2 ] ) ? $PRESERVATION_STORAGE[ $var_name ][ $key ][ $key2 ] : $default;
		}
	}
}

if ( ! function_exists( 'preservation_storage_set_array' ) ) {
	/**
	 ...
	 */
	function preservation_storage_set_array( $var_name, $key, $value ) {
		global $PRESERVATION_STORAGE;
		if ( ! isset( $PRESERVATION_STORAGE[ $var_name ] ) ) {
			$PRESERVATION_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			$PRESERVATION_STORAGE[ $var_name ][] = $value;
		} else {
...
if ( ! function_exists( 'preservation_storage_set_array2' ) ) {
	/**
	 ...
	 * @param mixed  $value     A new value for the specified element of the array.
	 */
	function preservation_storage_set_array2( $var_name, $key, $key2, $value ) {
		global $PRESERVATION_STORAGE;
		if ( ! isset( $PRESERVATION_STORAGE[ $var_name ] ) ) {
			$PRESERVATION_STORAGE[ $var_name ] = array();
		}
		if ( ! isset( $PRESERVATION_STORAGE[ $var_name ][ $key ] ) ) {
			$PRESERVATION_STORAGE[ $var_name ][ $key ] = array();
		}
		if ( '' === $key2 ) {
			$PRESERVATION_STORAGE[ $var_name ][ $key ][] = $value;
		} else {
			$PRESERVATION_STORAGE[ $var_name ][ $key ][ $key2 ] = $value;
		}
	}
}

if ( ! function_exists( 'preservation_storage_merge_array' ) ) {
	/**
	 ...
	 */
	function preservation_storage_merge_array( $var_name, $key, $value ) {
		global $PRESERVATION_STORAGE;
		if ( ! isset( $PRESERVATION_STORAGE[ $var_name ] ) ) {
			$PRESERVATION_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			$PRESERVATION_STORAGE[ $var_name ] = array_merge( $PRESERVATION_STORAGE[ $var_name ], $value );
		} else {
			$PRESERVATION_STORAGE[ $var_name ][ $key ] = array_merge( $PRESERVATION_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'preservation_storage_set_array_after' ) ) {
	/**
	 ...
	 */
	function preservation_storage_set_array_after( $var_name, $after, $key, $value = '' ) {
		global $PRESERVATION_STORAGE;
		if ( ! isset( $PRESERVATION_STORAGE[ $var_name ] ) ) {
			$PRESERVATION_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			preservation_array_insert_after( $PRESERVATION_STORAGE[ $var_name ], $after, $key );
		} else {
			preservation_array_insert_after( $PRESERVATION_STORAGE[ $var_name ], $after, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'preservation_storage_set_array_before' ) ) {
	/**
	 * Insert new elements to the array in the global theme storage
	 ...
	 * @param mixed        $value     A new value to insert (if the argument $key is a string).
	 */
	function preservation_storage_set_array_before( $var_name, $before, $key, $value = '' ) {
		global $PRESERVATION_STORAGE;
		if ( ! isset( $PRESERVATION_STORAGE[ $var_name ] ) ) {
			$PRESERVATION_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			preservation_array_insert_before( $PRESERVATION_STORAGE[ $var_name ], $before, $key );
		} else {
			preservation_array_insert_before( $PRESERVATION_STORAGE[ $var_name ], $before, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'preservation_storage_push_array' ) ) {
	/**
	 ...
	 */
	function preservation_storage_push_array( $var_name, $key, $value ) {
		global $PRESERVATION_STORAGE;
		if ( ! isset( $PRESERVATION_STORAGE[ $var_name ] ) ) {
			$PRESERVATION_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			array_push( $PRESERVATION_STORAGE[ $var_name ], $value );
		} else {
			if ( ! isset( $PRESERVATION_STORAGE[ $var_name ][ $key ] ) ) {
				$PRESERVATION_STORAGE[ $var_name ][ $key ] = array();
			}
			array_push( $PRESERVATION_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'preservation_storage_pop_array' ) ) {
	/**
	 ...
	 */
	function preservation_storage_pop_array( $var_name, $key = '', $defa = '' ) {
		global $PRESERVATION_STORAGE;
		$rez = $defa;
		if ( '' === $key ) {
			if ( isset( $PRESERVATION_STORAGE[ $var_name ] ) && is_array( $PRESERVATION_STORAGE[ $var_name ] ) && count( $PRESERVATION_STORAGE[ $var_name ] ) > 0 ) {
				$rez = array_pop( $PRESERVATION_STORAGE[ $var_name ] );
			}
		} else {
			if ( isset( $PRESERVATION_STORAGE[ $var_name ][ $key ] ) && is_array( $PRESERVATION_STORAGE[ $var_name ][ $key ] ) && count( $PRESERVATION_STORAGE[ $var_name ][ $key ] ) > 0 ) {
				$rez = array_pop( $PRESERVATION_STORAGE[ $var_name ][ $key ] );
			}
		}
		return $rez;
	}
...
if ( ! function_exists( 'preservation_storage_inc_array' ) ) {
	/**
	 ...
	 * @param int          $value     Optional. A value for increment. Default is 1. Specify -1 to decrement or any legal number value.
	 */
	function preservation_storage_inc_array( $var_name, $key, $value = 1 ) {
		global $PRESERVATION_STORAGE;
		if ( ! isset( $PRESERVATION_STORAGE[ $var_name ] ) ) {
			$PRESERVATION_STORAGE[ $var_name ] = array();
		}
		if ( empty( $PRESERVATION_STORAGE[ $var_name ][ $key ] ) ) {
			$PRESERVATION_STORAGE[ $var_name ][ $key ] = 0;
		}
		$PRESERVATION_STORAGE[ $var_name ][ $key ] += $value;
	}
}

if ( ! function_exists( 'preservation_storage_concat_array' ) ) {
	/**
	 ...
	 * @param int          $value     A value to concatenate.
	 */
	function preservation_storage_concat_array( $var_name, $key, $value ) {
		global $PRESERVATION_STORAGE;
		if ( ! isset( $PRESERVATION_STORAGE[ $var_name ] ) ) {
			$PRESERVATION_STORAGE[ $var_name ] = array();
		}
		if ( empty( $PRESERVATION_STORAGE[ $var_name ][ $key ] ) ) {
			$PRESERVATION_STORAGE[ $var_name ][ $key ] = '';
		}
		$PRESERVATION_STORAGE[ $var_name ][ $key ] .= $value;
	}
}

if ( ! function_exists( 'preservation_storage_call_obj_method' ) ) {
	/**
	 ...
	 */
	function preservation_storage_call_obj_method( $var_name, $method, $param = null ) {
		global $PRESERVATION_STORAGE;
		if ( null === $param ) {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $PRESERVATION_STORAGE[ $var_name ] ) ? $PRESERVATION_STORAGE[ $var_name ]->$method() : '';
		} else {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $PRESERVATION_STORAGE[ $var_name ] ) ? $PRESERVATION_STORAGE[ $var_name ]->$method( $param ) : '';
		}
	}
}

if ( ! function_exists( 'preservation_storage_get_obj_property' ) ) {
	/**
	 ...
	 */
	function preservation_storage_get_obj_property( $var_name, $prop, $default = '' ) {
		global $PRESERVATION_STORAGE;
		return ! empty( $var_name ) && ! empty( $prop ) && isset( $PRESERVATION_STORAGE[ $var_name ]->$prop ) ? $PRESERVATION_STORAGE[ $var_name ]->$prop : $default;
	}
}
