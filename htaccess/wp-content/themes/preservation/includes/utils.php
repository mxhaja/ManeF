...
if ( ! function_exists( 'preservation_array_get_first' ) ) {
	/**
	 ...
	 */
	function preservation_array_get_first( &$arr, $key = true ) {
		foreach ( $arr as $k => $v ) {
			break;
		}
		return $key ? $k : $v;
...
if ( ! function_exists( 'preservation_array_get_keys_by_value' ) ) {
	/**
	 ...
	 */
	function preservation_array_get_keys_by_value( $arr, $value = 1 ) {
		if ( ! is_array( $arr ) ) {
...
if ( ! function_exists( 'preservation_array_delete_by_value' ) ) {
	/**
	 ...
	 */
	function preservation_array_delete_by_value( $arr, $value ) {
...
if ( ! function_exists( 'preservation_array_from_list' ) ) {
	/**
	 ...
	 */
	function preservation_array_from_list( $arr ) {
		$new = array();
		foreach ( $arr as $v ) {
			$new[ $v ] = $v;
		}
...
if ( ! function_exists( 'preservation_array_slice' ) ) {
	/**
	 ...
	 */
	function preservation_array_slice( $arr, $from, $to='' ) {
...
if ( ! function_exists( 'preservation_array_merge' ) ) {
	/**
	 * Merge arrays and lists (preserve a numeric indexes).
	 *
	 * For example:
	 *
	 * preservation_array_merge( [ 1, 2, 3, 4 ], [ 'one', 'two' ] )  // result: [ 'one', 'two', 3, 4 ]
	 *
	 * preservation_array_merge( [ 'a' => 'A', 'b' => 'B' ], [ 'one', 'two', 'a' => 'AA' ] )  // result: [ 'a' => 'AA', 'b' => 'B', 0 => 'one', 1 => 'two' ]
	 *
	 ...
	 */
	function preservation_array_merge( $a1, $a2 ) {
...
if ( ! function_exists( 'preservation_array_insert_after' ) ) {
	/**
	 ...
	 */
	function preservation_array_insert_after( &$haystack, $needle, $stuff ) {
		...
		for ( $i = 2; $i < func_num_args(); ++$i ) {
			...
			if ( is_array( $arg ) ) {
				if ( 2 == $i ) {
					...
					$new_array = preservation_array_merge( $new_array, $arg );
				}
			} else {
				$new_array[] = $arg;
			}
		...
		$haystack = is_int( $needle )
						...
						: preservation_array_merge( array_slice( $haystack, 0, $i, true ), $new_array, array_slice( $haystack, $i, null, true ) );

		return $i;
	}
}

if ( ! function_exists( 'preservation_array_insert_before' ) ) {
	/**
	 * Inserts any number of scalars or arrays at the point in the haystack
	 ...
	 * @return int               The index at which $needle was found.
	 */
	function preservation_array_insert_before( &$haystack, $needle, $stuff ) {
		if ( ! is_array( $haystack ) ) {
			return -1;
		}
...
if ( ! function_exists( 'preservation_array_get_by_key' ) ) {
	/**
	 ...
	 */
	function preservation_array_get_by_key( &$arr, $key, $value, $key2 = '', $default = '' ) {
		$rez = $default;
		foreach ( $arr as $v ) {
...
if ( ! function_exists( 'preservation_generate_id' ) ) {
	/**
	 ...
	 */
	function preservation_generate_id( $prefix = '' ) {
		return $prefix . str_replace( '.', '', mt_rand() );
	}
}

if ( ! function_exists( 'preservation_get_tag' ) ) {
	/**
	 ...
	 */
	function preservation_get_tag( $text, $tag_start, $tag_end = '' ) {
		$val       = '';
...
if ( ! function_exists( 'preservation_get_tag_attrib' ) ) {
	/**
	 ...
	 */
	function preservation_get_tag_attrib( $text, $tag, $attr ) {
		$val       = '';
...
if ( ! function_exists( 'preservation_get_css_position_from_values' ) ) {
	/**
	 ...
	 */
	function preservation_get_css_position_from_values( $top = '', $right = '', $bottom = '', $left = '', $width = '', $height = '' ) {
		if ( ! is_array( $top ) ) {
		...
		foreach ( $top as $k => $v ) {
			...
			if ( '' != $v ) {
				$output .= ( 'width' == $k ? 'width' : ( 'height' == $k ? 'height' : 'margin-' . esc_attr( $k ) ) ) . ':' . esc_attr( preservation_prepare_css_value( $v ) ) . ( '!' == $imp ? ' !important' : '' ) . ';';
			}
		}
		return $output;
	}
...
if ( ! function_exists( 'preservation_prepare_css_value' ) ) {
	/**
	 ...
	 */
	function preservation_prepare_css_value( $val ) {
		if ( '' !== $val ) {
...
if ( ! function_exists( 'preservation_parse_icons_classes' ) ) {
	/**
	 ...
	 */
	function preservation_parse_icons_classes( $css ) {
		$rez = array();
		if ( ! file_exists( $css ) ) {
			return $rez;
		}
		$file = preservation_fga( $css );
...
if ( ! function_exists( 'preservation_get_column_class' ) ) {
	/**
	 ...
	 */
	function preservation_get_column_class( $num, $all, $all_tablet = '', $all_mobile = '' ) {
...
if ( ! function_exists( 'preservation_stripslashes' ) ) {
	/**
	 ...
	 */
	function preservation_stripslashes( $val ) {
		static $magic = 0;
		if ( 0 === $magic ) {
		...
		if ( is_array( $val ) ) {
			foreach ( $val as $k => $v ) {
				$val[ $k ] = preservation_stripslashes( $v );
			}
		} else {
...
if ( ! function_exists( 'preservation_get_value_gp' ) ) {
	/**
	 ...
	 */
	function preservation_get_value_gp( $name, $defa = '' ) {
		if ( isset( $_GET[ $name ] ) ) {
...
if ( ! function_exists( 'preservation_get_value_gpc' ) ) {
	/**
	 ...
	 */
	function preservation_get_value_gpc( $name, $defa = '' ) {
		if ( isset( $_GET[ $name ] ) ) {
			$rez = wp_unslash( $_GET[ $name ] );
		} elseif ( isset( $_POST[ $name ] ) ) {
			$rez = wp_unslash( $_POST[ $name ] );
...
if ( ! function_exists( 'preservation_get_value_gps' ) ) {
	/**
	 ...
	 */
	function preservation_get_value_gps( $name, $defa = '' ) {
		global $wp_session;
		if ( isset( $_GET[ $name ] ) ) {
			$rez = wp_unslash( $_GET[ $name ] );
		} elseif ( isset( $_POST[ $name ] ) ) {
...
if ( ! function_exists( 'preservation_get_session_value' ) ) {
	/**
	 ...
	 */
	function preservation_get_session_value( $name, $defa = '' ) {
		global $wp_session;
		return isset( $wp_session[ $name ] ) ? $wp_session[ $name ] : $defa;
	}
}

if ( ! function_exists( 'preservation_set_session_value' ) ) {
	/**
	 ...
	 */
	function preservation_set_session_value( $name, $value ) {
		global $wp_session;
		$wp_session[ $name ] = $value;
	}
}

if ( ! function_exists( 'preservation_del_session_value' ) ) {
	/**
	 * Delete a saved variable from the current session.
	 *
	 * @param string $name   A name to to delete from the session.
	 */
	function preservation_del_session_value( $name ) {
		global $wp_session;
		unset( $wp_session[ $name ] );
	}
}

if ( ! function_exists( 'preservation_get_cookie' ) ) {
	/**
	 ...
	 */
	function preservation_get_cookie( $name, $defa = '' ) {
		return preservation_stripslashes( isset( $_COOKIE[ $name ] ) ? $_COOKIE[ $name ] : $defa );
	}
}

// Set a cookie - wrapper for setcookie using WP constants.
if ( ! function_exists( 'preservation_set_cookie' ) ) {
	/**
	 ...
	 */
	function preservation_set_cookie( $name, $value, $expire = 0, $secure = false, $httponly = false  ) {
		if ( ! headers_sent() ) {
			if ( defined( 'PHP_VERSION_ID' ) && PHP_VERSION_ID >= 70300 ) {
				$rez = setcookie(
							$name,
							$value,
							apply_filters( 'preservation_filter_cookie_options', array(
								'expires'  => $expire,
								'path'     => defined( 'COOKIEPATH' ) && ! empty( COOKIEPATH ) ? COOKIEPATH : '/',
								'domain'   => defined( 'COOKIE_DOMAIN' ) && ! empty( COOKIE_DOMAIN ) ? COOKIE_DOMAIN : '',	//preservation_get_domain_from_url( get_home_url() ),
								'secure'   => $secure,
								'httponly' => $httponly,
				...
				$rez = setcookie(
							...
							defined( 'COOKIE_DOMAIN' ) && ! empty( COOKIE_DOMAIN ) ? COOKIE_DOMAIN : '',	//preservation_get_domain_from_url( get_home_url() ),
							$secure,
							$httponly
						);
			}
			...
			trigger_error( sprintf( __( '%1$s cookie cannot be set - headers already sent by %2$s on line %3$s', 'preservation' ), $name, $file, $line ), E_USER_NOTICE ); // @codingStandardsIgnoreLine
		}
	}
}
...
if ( ! function_exists( 'preservation_hex2rgb' ) ) {
	/**
	 ...
	 */
	function preservation_hex2rgb( $hex ) {
...
if ( ! function_exists( 'preservation_hex2rgba' ) ) {
	/**
	 ...
	 */
	function preservation_hex2rgba( $hex, $alpha ) {
		$rgb = preservation_hex2rgb( $hex );
		return 'rgba(' . intval( $rgb['r'] ) . ',' . intval( $rgb['g'] ) . ',' . intval( $rgb['b'] ) . ',' . floatval( $alpha ) . ')';
	}
}

if ( ! function_exists( 'preservation_hex2hsb' ) ) {
	/**
	 ...
	 */
	function preservation_hex2hsb( $hex, $h = 0, $s = 0, $b = 0 ) {
		$hsb      = preservation_rgb2hsb( preservation_hex2rgb( $hex ) );
...
if ( ! function_exists( 'preservation_rgb2hsb' ) ) {
	/**
	 ...
	 * @return int[]      An array with HSB components in the format: ['h' => hue_value, 's' => saturation_value, 'b' => brightness_value].
	 */
	function preservation_rgb2hsb( $rgb ) {
		$hsb      = array();
...
if ( ! function_exists( 'preservation_hsb2rgb' ) ) {
	/**
	 ...
	 * @return int[]      An array with RGB components in the format: ['r' => red_value, 'g' => green_value, 'b' => blue_value].
	 */
	function preservation_hsb2rgb( $hsb ) {
		$rgb = array();
		$h   = round( $hsb['h'] );
...
if ( ! function_exists( 'preservation_rgb2hex' ) ) {
	/**
	 ...
	 */
	function preservation_rgb2hex( $rgb ) {
		$hex = array(
			dechex( $rgb['r'] ),
			dechex( $rgb['g'] ),
			dechex( $rgb['b'] ),
...
if ( ! function_exists( 'preservation_hsb2hex' ) ) {
	/**
	 ...
	 * @return string     A string with a hex color presentation in the format '#RRGGBB'.
	 */
	function preservation_hsb2hex( $hsb ) {
		return preservation_rgb2hex( preservation_hsb2rgb( $hsb ) );
	}
}
...
//
if ( ! function_exists( 'preservation_date_to_sql' ) ) {
	/**
	 ...
	 */
	function preservation_date_to_sql($str) {
		if (trim($str)=='') return '';
		$str = strtr(trim($str),'/\.,','----');
...
if ( ! function_exists( 'preservation_format_price' ) ) {
	/**
	 ...
	 */
	function preservation_format_price( $price ) {
...
if ( ! function_exists( 'preservation_num2size' ) ) {
	/**
	 ...
	 */
	function preservation_num2size( $num ) {
		return $num > 1000 ? round( $num/1000, 0 ) . 'K' : $num;
	}
}

if ( ! function_exists( 'preservation_size2num' ) ) {
	/**
	 ...
	 */
	function preservation_size2num( $size, $full = false ) {
...
//
if ( ! function_exists( 'preservation_parse_num' ) ) {
	/**
	 ...
	 */
	function preservation_parse_num( $str ) {
...
if ( ! function_exists( 'preservation_prepare_macros' ) ) {
	/**
	 ...
	 * @return string      A processed string.
	 */
	function preservation_prepare_macros( $str ) {
		return str_replace(
			array( '{{', '}}', '((', '))', '||' ),
			array( '<i>', '</i>', '<b>', '</b>', '<br>' ),
			$str
...
if ( ! function_exists( 'preservation_remove_macros' ) ) {
	/**
	 ...
	 * @return string      A processed string.
	 */
	function preservation_remove_macros( $str ) {
		return str_replace(
			array( '{{', '}}', '((', '))', '||' ),
			array( '', '', '', '', ' ' ),
			$str
...
if ( ! function_exists( 'preservation_is_on' ) ) {
	/**
	 ...
	 */
	function preservation_is_on( $prm ) {
		return is_array( $prm )
					? count( $prm ) > 0
...
if ( ! function_exists( 'preservation_is_off' ) ) {
	/**
	 ...
	 */
	function preservation_is_off( $prm ) {
		return is_array( $prm )
					? count( $prm ) == 0
					: empty( $prm )
...
if ( ! function_exists( 'preservation_is_inherit' ) ) {
	/**
	 ...
	 */
	function preservation_is_inherit( $prm ) {
		return ! is_array( $prm ) && in_array( strtolower( $prm ), array( 'inherit' ) );
	}
}

if ( ! function_exists( 'preservation_strshort' ) ) {
	/**
	 ...
	 */
	function preservation_strshort( $str, $maxlength, $add = '&hellip;' ) {
		if ( 0 >= $maxlength ) {
			return '';
		}
		$str = preservation_strip_tags( $str );
...
if ( ! function_exists( 'preservation_strwords' ) ) {
	/**
	 ...
	 * @return string            A truncated string.
	 */
	function preservation_strwords( $str, $maxlength, $add = '&hellip;' ) {
		if ( $maxlength <= 0 ) {
			return '';
		}
		$words = explode( ' ', $str );
...
if ( ! function_exists( 'preservation_strip_tags' ) ) {
	/**
	 ...
	 */
	function preservation_strip_tags( $str ) {
...
if ( ! function_exists( 'preservation_excerpt' ) ) {
	/**
	 ...
	 */
	function preservation_excerpt( $str, $maxlength, $add = '&hellip;' ) {
		if ( $maxlength <= 0 ) {
			return '';
		}
		return preservation_strwords( preservation_strip_tags( $str ), $maxlength, $add );
	}
}

if ( ! function_exists( 'preservation_unserialize' ) ) {
	/**
	 ...
	 */
	function preservation_unserialize( $str ) {
...
if ( ! function_exists( 'preservation_str_replace' ) ) {
	/**
	 ...
	 */
	function preservation_str_replace( $from, $to, $str ) {
		if ( is_array( $str ) ) {
			foreach ( $str as $k => $v ) {
				$str[ $k ] = preservation_str_replace( $from, $to, $v );
			}
			...
			if ( is_serialized( $str ) ) {
				$str = serialize( preservation_str_replace( $from, $to, preservation_unserialize( $str ) ) );
			} else {
...
if ( ! function_exists( 'preservation_str_replace_once' ) ) {
	/**
	 ...
	 * @return string             A processed string.
	 */
	function preservation_str_replace_once( $from, $to, $str ) {
		$rez = '';
		if ( ! is_array( $from ) ) {
			$from = array( $from );
		}
...
if ( ! function_exists( 'preservation_tags_count' ) ) {
	/**
	 ...
	 */
	function preservation_tags_count( $str, $tag ) {
		$cnt = 0;
...
if ( ! function_exists( 'preservation_getimagesize' ) ) {
	/**
	 ...
	 */
	function preservation_getimagesize( $url, $echo = false ) {
		$img_size = false;
		$img_path = preservation_is_url( $url ) ? preservation_url_to_local_path( $url ) : $url;
...
if ( ! function_exists( 'preservation_clear_thumb_size' ) ) {
	/**
	 ...
	 */
	function preservation_clear_thumb_size( $url, $remove_protocol = true ) {
		$pi = pathinfo( $url );
		if ( $remove_protocol ) {
			$pi['dirname'] = preservation_remove_protocol_from_url( $pi['dirname'], false );
		}
...
if ( ! function_exists( 'preservation_add_thumb_size' ) ) {
	/**
	 ...
	 */
	function preservation_add_thumb_size( $url, $thumb_size, $check_exists = true ) {
		...
		$new_url = preservation_remove_protocol_from_url( ( ! empty( $pi['dirname'] ) ? $pi['dirname'] . '/' : '' ) . $pi['filename'] . ( ! empty( $pi['extension'] ) ? '.' . $pi['extension'] : '' ) );

		// Check exists
		if ( $check_exists ) {
			$uploads_info = wp_upload_dir();
			$uploads_url = preservation_remove_protocol_from_url( $uploads_info['baseurl'] );
			...
			if ( strpos( $new_url, $uploads_url ) !== false ) {
				if ( ! file_exists( str_replace( $uploads_url, $uploads_dir, $new_url ) ) ) {
					$new_url = preservation_remove_protocol_from_url( $url );
				}
			} else {
				$new_url = preservation_remove_protocol_from_url( $url );
			}
...
if ( ! function_exists( 'preservation_get_thumb_size' ) ) {
	/**
	 * Return a theme-specific image thumb size name with a prefix and a multiplier (if a visitor have a retina device).
	 *
	 * For example:
	 *
	 * 'masonry' -> 'preservation-thumb-masonry' or 'preservation-thumb-masonry-@retina'
	 *
	 ...
	 */
	function preservation_get_thumb_size( $ts ) {
		$retina = preservation_get_retina_multiplier() > 1 ? '-@retina' : '';
		$is_external = apply_filters(
							'preservation_filter_is_external_thumb_size',
							in_array( $ts,
										apply_filters(
											'preservation_filter_external_thumb_sizes',
		...
		$is_internal = strpos( $ts, 'preservation-thumb-' ) === 0 || strpos( $ts, 'trx_addons-thumb-' ) === 0;
		return apply_filters(
					'preservation_filter_get_thumb_size',
					( $is_external || $is_internal ? '' : 'preservation-thumb-' )
					. $ts
					. ( $is_internal ? $retina : '' )
				);
	}
...
if ( ! function_exists( 'preservation_get_closest_thumb_size' ) ) {
	 ...
	 */
	function preservation_get_closest_thumb_size( $old_size, $dim, $prefix = '' ) {
...
if ( ! function_exists( 'preservation_get_attachment_url' ) ) {
	/**
	 ...
	 * @return string
	 */
	function preservation_get_attachment_url( $image_id, $size = 'full' ) {
		...
		if ( is_numeric( $image_id ) && (int) $image_id > 0 ) {
			...
			$image_id = preservation_add_thumb_size( $image_id, $size );
		}
		return $image_id;
	}
}

if ( ! function_exists( 'preservation_get_post_image' ) ) {
	/**
	 ...
	 */
	function preservation_get_post_image( $post_text = '', $src = true ) {
		global $post;
		$img = '';
		if ( empty( $post_text ) ) {
...
if ( ! function_exists( 'preservation_get_post_audio' ) ) {
	/**
	 ...
	 * @return string           Founded URL (or a whole tag) or an empty string if found nothing.
	 */
	function preservation_get_post_audio( $post_text = '', $src = true ) {
		global $post;
		$img = preservation_get_post_audio_list_first( ! $src );
		if ( is_array( $img ) ) {
			if ( ! empty( $img['url'] ) ) {
				$img = $img['url'];
			} else if ( ! empty( $img['embed'] ) ) {
				$img = preservation_get_post_iframe( $img['embed'], true );
			} else {
				$img = '';
			}
		}
		if ( empty( $img ) ) {
			...
			if ( $src ) {
				...
				$img = preservation_get_tag( $post_text, '<audio', '</audio>' );
				if ( empty( $img ) ) {
					$img = do_shortcode( preservation_get_tag( $post_text, '[audio', '[/audio]' ) );
				}
				if ( empty( $img ) ) {
					$img = preservation_get_tag_attrib( $post_text, '[trx_widget_audio]', 'url' );
...
if ( ! function_exists( 'preservation_get_post_audio_list' ) ) {
	/**
	 ...
	 *                     ]
	 */
	function preservation_get_post_audio_list( $args=array() ) {
		return function_exists( 'trx_addons_get_post_audio_list' ) ? trx_addons_get_post_audio_list( $args ) : array();
	}
}

if ( ! function_exists( 'preservation_get_post_audio_list_first' ) ) {
	/**
	 ...
	 *                     or the URL only
	 */
	function preservation_get_post_audio_list_first( $src = false, $args = array() ) {
		return function_exists( 'trx_addons_get_post_audio_list_first' ) ? trx_addons_get_post_audio_list_first( $src, $args ) : '';
	}
}

if ( ! function_exists( 'preservation_get_post_video' ) ) {
	/**
	 ...
	 * @return string           Founded URL (or a whole tag) or an empty string if found nothing.
	 */
	function preservation_get_post_video( $post_text = '', $src = true ) {
		global $post;
		$img = preservation_get_post_video_list_first( ! $src );
		if ( is_array( $img ) ) {
			if ( ! empty( $img['video_url'] ) ) {
				$img = $img['video_url'];
			} else if ( ! empty( $img['video_embed'] ) ) {
				$img = preservation_get_post_iframe( $img['video_embed'], true );
			} else {
				$img = '';
			}
		}
		if ( empty( $img ) ) {
			...
			if ( $src ) {
				...
				$img = preservation_get_tag( $post_text, '<video', '</video>' );
				if ( empty( $img ) ) {
					$sc = preservation_get_tag( $post_text, '[video', '[/video]' );
					if ( empty( $sc ) ) {
						$sc = preservation_get_tag( $post_text, '[trx_widget_video', '' );
					}
					if ( ! empty( $sc ) ) {
						$img = do_shortcode( $sc );
					}
					if ( empty( $img ) && preg_match_all( '/<!-- wp\\:trx-addons\\/video.+"link"\\:"([^"]*)"/is', $post_text, $matches ) ) {
						$img = preservation_get_embed_video( $matches[1][0] );
					}
					if ( empty( $img ) && preg_match_all( '/<!-- wp:core-embed\\/(youtube|vimeo|dailymotion|facebook).+"url":"([^"]*)"/is', $post_text, $matches ) ) {
						$img = preservation_get_embed_video( $matches[2][0] );	// , true
					}
					if ( empty( $img ) && preg_match_all( '/<!-- wp:embed {"url":"([^"]*(youtube|vimeo|dailymotion|facebook)[^"]*)"/is', $post_text, $matches ) ) {
						$img = preservation_get_embed_video( $matches[1][0] );	// , true
					}
		...
		return apply_filters( 'preservation_filter_get_post_video', $img );
	}
}

if ( ! function_exists( 'preservation_get_post_video_list' ) ) {
	/**
	 ...
	 *                     ]
	 */
	function preservation_get_post_video_list( $args=array() ) {
		return function_exists( 'trx_addons_get_post_video_list' ) ? trx_addons_get_post_video_list( $args ) : array();
	}
}

if ( ! function_exists( 'preservation_get_post_video_list_first' ) ) {
	/**
	 ...
	 *                     or the URL only
	 */
	function preservation_get_post_video_list_first( $src = false, $args = array() ) {
		return function_exists( 'trx_addons_get_post_video_list_first' ) ? trx_addons_get_post_video_list_first( $src, $args ) : '';
	}
}

if ( ! function_exists( 'preservation_get_post_iframe' ) ) {
	/**
	 ...
	 * @return string           Founded URL (or a whole tag) or an empty string if found nothing.
	 */
	function preservation_get_post_iframe( $post_text = '', $src = true ) {
		global $post;
		$img = '';
		$tag = preservation_get_embed_video_tag_name();
		if ( empty( $post_text ) ) {
			$post_text = $post->post_content;
		}
		if ( $src ) {
			...
			$img = preservation_get_tag( $post_text, '<' . esc_html( $tag ), '</' . esc_html( $tag ) . '>' );
		}
		return apply_filters( 'preservation_filter_get_post_iframe', $img );
	}
}

if ( ! function_exists( 'preservation_get_embed_video' ) ) {
	/**
	 ...
	 */
	function preservation_get_embed_video( $video, $use_wp_embed = false ) {
		global $wp_embed;
		if ( $use_wp_embed && is_object( $wp_embed ) ) {
			$embed_video = do_shortcode( $wp_embed->run_shortcode( '[embed]' . trim( $video ) . '[/embed]' ) );
			// $embed_video = preservation_make_video_autoplay( $embed_video );
		} else if ( preservation_is_from_uploads( $video ) ) {
			...
			$video = preservation_add_to_url( $video, preservation_get_embed_video_parameters() );
			$tag   = preservation_get_embed_video_tag_name();
			// Calc video width and height (with ratio 16:9)
			$ratio  = explode( ':', apply_filters( 'preservation_filter_video_ratio', '16:9' ) );
			$width  = preservation_get_content_width();
			$height = round( $width / $ratio[0] * $ratio[1] );
			$dim    = apply_filters( 'preservation_filter_video_dimensions', array(
									'width' => $width,
									'height' => $height
									) );
...
if ( ! function_exists( 'preservation_get_embed_video_tag_name' ) ) {
	/**
	 * Return a name of the tag-container to embed video.
	 *
	 * @return string  A tag name to embed video.
	 */
	function preservation_get_embed_video_tag_name() {
...
if ( ! function_exists( 'preservation_make_video_autoplay' ) ) {
	/**
	 ...
	 */
	function preservation_make_video_autoplay( $video, $muted = false ) {
		if ( strpos( $video, '<video' ) !== false ) {
			...
			if ( false !== $pos ) {
				if ( preg_match( '/(<' . esc_html( $tag ) . '[^>]*src=[\'"])([^\'"]+)([\'"][^>]*>)(.*)/is', $video, $matches ) ) {
					...
					if ( ! empty( $matches[2] ) ) {
						// Add parameters to the 'src'
						$matches[2] = preservation_add_to_url(
							$matches[2],
							array_merge(
								array(
								...
								$muted
									? array_merge(
										preservation_get_embed_video_parameters(),
										array(
											'muted' => 1,
											'controls' => 0,
											'background' => 1,
...
if ( ! function_exists( 'preservation_is_from_uploads' ) ) {
	/**
	 ...
	 */
	function preservation_is_from_uploads( $url ) {
		$url          = preservation_remove_protocol_from_url( $url );
		$parts        = explode( '?', $url );
		$url          = $parts[0];
		$uploads_info = wp_upload_dir();
		$uploads_url  = preservation_remove_protocol_from_url( $uploads_info['baseurl'] );
...
if ( ! function_exists( 'preservation_is_youtube_url' ) ) {
	/**
	 ...
	 */
	function preservation_is_youtube_url( $url ) {
		return strpos( $url, 'youtu.be' ) !== false || strpos( $url, 'youtube.com' ) !== false;
	}
}

if ( ! function_exists( 'preservation_is_vimeo_url' ) ) {
	/**
	 ...
	 */
	function preservation_is_vimeo_url( $url ) {
...
if ( ! function_exists( 'preservation_init_filesystem' ) ) {
	add_action( 'after_setup_theme', 'preservation_init_filesystem', 0 );
	/**
	 ...
	 */
	function preservation_init_filesystem( $force = false ) {
...
if ( ! function_exists( 'preservation_fpc' ) ) {
	/**
	 ...
	 */
	function preservation_fpc( $file, $data, $flag = 0 ) {
		if ( ! empty( $file ) ) {
			if ( function_exists( 'trx_addons_fpc' ) ) {
				...
				if ( isset( $wp_filesystem ) && is_object( $wp_filesystem ) ) {
					...
					if ( preservation_is_on( preservation_get_theme_option( 'debug_mode' ) ) ) {
						...
						throw new Exception( sprintf( esc_html__( 'WP Filesystem is not initialized! Put contents to the file "%s" failed', 'preservation' ), $file ) );
					}
				}
			}
		}
...
if ( ! function_exists( 'preservation_fgc' ) ) {
	/**
	 ...
	 * @throws Exception    Throw an Exception if the object $wp_filesystem is not initialized yet.
	 */
	function preservation_fgc( $file ) {
		if ( ! empty( $file ) ) {
			if ( function_exists( 'trx_addons_fgc' ) ) {
				...
				global $wp_filesystem;
				if ( isset( $wp_filesystem ) && is_object( $wp_filesystem ) ) {
					$file = substr( $file, 0, 2 ) == '//' ? preservation_add_protocol( $file ) : str_replace( ABSPATH, $wp_filesystem->abspath(), $file );
					return $wp_filesystem->get_contents( $file );
				} else {
					if ( preservation_is_on( preservation_get_theme_option( 'debug_mode' ) ) ) {
						// Translators: Add the file name to the message
						throw new Exception( sprintf( esc_html__( 'WP Filesystem is not initialized! Get contents from the file "%s" failed', 'preservation' ), $file ) );
					}
				}
			}
		}
...
if ( ! function_exists( 'preservation_fga' ) ) {
	/**
	 ...
	 * @throws Exception    Throw an Exception if the object $wp_filesystem is not initialized yet.
	 */
	function preservation_fga( $file ) {
		if ( ! empty( $file ) ) {
			if ( function_exists( 'trx_addons_fga' ) ) {
				...
				if ( isset( $wp_filesystem ) && is_object( $wp_filesystem ) ) {
					...
					if ( preservation_is_on( preservation_get_theme_option( 'debug_mode' ) ) ) {
						// Translators: Add the file name to the message
						throw new Exception( sprintf( esc_html__( 'WP Filesystem is not initialized! Get rows from the file "%s" failed', 'preservation' ), $file ) );
					}
				}
			}
		}
...
if ( ! function_exists( 'preservation_mkdir' ) ) {
	/**
	 ...
	 */
	function preservation_mkdir( $path ) {
		if ( ! empty( $path ) ) {
				...
				if ( isset( $wp_filesystem ) && is_object( $wp_filesystem ) ) {
					...
					if ( ! $wp_filesystem->is_dir( $path ) ) {
						...
						if ( ! wp_mkdir_p( $path ) ) {
							if ( preservation_is_on( preservation_get_theme_option( 'debug_mode' ) ) ) {
								// Translators: Add the file name to the message
								throw new Exception( sprintf( esc_html__( 'Create a folder "%s" failed', 'preservation' ), $path ) );
							}
						} else {
							return true;
						}
					...
					if ( preservation_is_on( preservation_get_theme_option( 'debug_mode' ) ) ) {
						// Translators: Add the file name to the message
						throw new Exception( sprintf( esc_html__( 'WP Filesystem is not initialized! Create a folder "%s" failed', 'preservation' ), $path ) );
					}
				}
			}
		}
...
if ( ! function_exists( 'preservation_unlink' ) ) {
	/**
	 ...
	 * @throws Exception        Throw an Exception if the object $wp_filesystem is not initialized yet or if error occurs.
	 */
	function preservation_unlink( $path, $recursive = true, $type = 'd' ) {
		if ( ! empty( $path ) ) {
			if ( function_exists( 'trx_addons_unlink' ) ) {
				...
				if ( isset( $wp_filesystem ) && is_object( $wp_filesystem ) ) {
					...
					if ( preservation_is_on( preservation_get_theme_option( 'debug_mode' ) ) ) {
						// Translators: Add the file name to the message
						throw new Exception( sprintf( esc_html__( 'WP Filesystem is not initialized! Delete a file/folder "%s" failed', 'preservation' ), $path ) );
					}
				}
			}
		}
...
if ( ! function_exists( 'preservation_copy' ) ) {
	/**
	 ...
	 * @throws Exception  Throw an Exception if the object $wp_filesystem is not initialized yet or if error occurs.
	 */
	function preservation_copy( $src, $dst ) {
		global $wp_filesystem;
		if ( ! empty( $src ) && ! empty( $dst ) ) {
			if ( function_exists( 'trx_addons_copy' ) ) {
				...
				$src = preservation_prepare_path( $src );
				$dst = preservation_prepare_path( $dst );
				return copy_dir( $src, $dst );
			} else {
				preservation_unlink( $dst );
				if ( is_dir( $src ) ) {
					if ( ! is_dir( $dst ) ) {
						preservation_mkdir( $dst );
					}
					$files = scandir( $src, SCANDIR_SORT_NONE );
					foreach ( $files as $file ) {
						if ( $file != "." && $file != ".." ) {
							preservation_copy( "$src/$file", "$dst/$file" );
						}
					}
					return true;
...
if ( ! function_exists( 'preservation_unzip_file' ) ) {
	/**
	 ...
	 */
	function preservation_unzip_file( $zip, $dest ) {
		if ( function_exists( 'trx_addons_unzip_file' ) ) {
			return trx_addons_unzip_file( $zip, $dest );
		}
		global $wp_filesystem;
		if ( empty( $wp_filesystem ) || ! is_object( $wp_filesystem ) ) {
			preservation_init_filesystem( true );
		}
		return unzip_file( $zip, $dest );
	}
}

if ( ! function_exists( 'preservation_retrieve_json' ) ) {
	/**
...
if ( ! function_exists( 'preservation_esc' ) ) {
	 ...
	 */
	function preservation_esc( $name ) {
		return str_replace( array( '\\', '~', '$', ':', ';', '+', '>', '<', '|', '"', "'", '`', "\xFF", "\x0A", "\x0D", '*', '?', '^' ), '/', trim( $name ) );
	}
}

if ( ! function_exists( 'preservation_prepare_path' ) ) {
	/**
	 ...
	 */
	function preservation_prepare_path( $path ) {
		return str_replace( array( '\\', '/' ), defined( 'DIRECTORY_SEPARATOR' ) ? DIRECTORY_SEPARATOR : '/', $path );
	}
}

if ( ! function_exists( 'preservation_check_min_file' ) ) {
	/**
	 ...
	 */
	function preservation_check_min_file( $file, $dir = '' ) {
		if ( empty( $dir ) ) {
			$dir = dirname( $file );
		}
		if ( substr( $file, -3 ) == '.js' ) {
			if ( substr( $file, -7 ) != '.min.js' && preservation_is_off( preservation_get_theme_option( 'debug_mode' ) ) ) {
			...
			if ( substr( $file, -8 ) != '.min.css' && preservation_is_off( preservation_get_theme_option( 'debug_mode' ) ) ) {
				$dir      = trailingslashit( $dir );
...
if ( ! function_exists( 'preservation_get_file_dir' ) ) {
	/**
	 ...
	 */
	function preservation_get_file_dir( $file, $return_url = false ) {
		// Use new WordPress functions (if present)
		if ( function_exists( 'get_theme_file_path' ) && ! PRESERVATION_ALLOW_SKINS && ! preservation_get_theme_setting( 'check_min_version', false ) ) {
		...
		// Otherwise (on WordPress older then 4.7.0) or theme use .min versions of .js and .css or theme use skins
			...
			$dir       = '';
			$theme_dir = apply_filters( 'preservation_filter_get_theme_file_dir', '', $file, $return_url );
			if ( '' != $theme_dir ) {
				$dir = $theme_dir;
			} elseif ( PRESERVATION_CHILD_DIR != PRESERVATION_THEME_DIR && file_exists( PRESERVATION_CHILD_DIR . ( $file ) ) ) {
				$dir = ( $return_url ? PRESERVATION_CHILD_URL : PRESERVATION_CHILD_DIR ) . preservation_check_min_file( $file, PRESERVATION_CHILD_DIR );
			} elseif ( file_exists( PRESERVATION_THEME_DIR . ( $file ) ) ) {
				$dir = ( $return_url ? PRESERVATION_THEME_URL : PRESERVATION_THEME_DIR ) . preservation_check_min_file( $file, PRESERVATION_THEME_DIR );
			}
		}
		return $dir;
	}
...
if ( ! function_exists( 'preservation_get_file_url' ) ) {
	/**
	 ...
	 *                          or in the main theme directory. If a file is not exists - return an empty string.
	 */
	function preservation_get_file_url( $file ) {
		return preservation_get_file_dir( $file, true );
	}
}

if ( ! function_exists( 'preservation_get_file_ext' ) ) {
	/**
	 ...
	 */
	function preservation_get_file_ext( $file ) {
		$ext = pathinfo( $file, PATHINFO_EXTENSION );
		return empty( $ext ) ? '' : $ext;
	}
}

if ( ! function_exists( 'preservation_get_file_name' ) ) {
	/**
	 ...
	 */
	function preservation_get_file_name( $file, $without_ext = true ) {
		$parts = pathinfo( $file );
		return ! empty( $parts['filename'] ) && $without_ext ? $parts['filename'] : $parts['basename'];
	}
}

if ( ! function_exists( 'preservation_get_folder_dir' ) ) {
	/**
	 ...
	 */
	function preservation_get_folder_dir( $folder, $return_url = false ) {
		if ( '/' == $folder[0] ) {
			$folder = substr( $folder, 1 );
		}
		$dir       = '';
		$theme_dir = apply_filters( 'preservation_filter_get_theme_folder_dir', '', $folder, $return_url );
		if ( '' != $theme_dir ) {
			$dir = $theme_dir;
		} elseif ( PRESERVATION_CHILD_DIR != PRESERVATION_THEME_DIR && is_dir( PRESERVATION_CHILD_DIR . ( $folder ) ) ) {
			$dir = ( $return_url ? PRESERVATION_CHILD_URL : PRESERVATION_CHILD_DIR ) . ( $folder );
		} elseif ( is_dir( PRESERVATION_THEME_DIR . ( $folder ) ) ) {
			$dir = ( $return_url ? PRESERVATION_THEME_URL : PRESERVATION_THEME_DIR ) . ( $folder );
		}
		return apply_filters( 'preservation_filter_get_folder_dir', $dir, $folder, $return_url );
	}
}

if ( ! function_exists( 'preservation_get_folder_url' ) ) {
	/**
	 ...
	 *                          or in the main theme directory. If a folder is not exists - return an empty string.
	 */
	function preservation_get_folder_url( $folder ) {
		return preservation_get_folder_dir( $folder, true );
	}
}

if ( ! function_exists( 'preservation_merge_js' ) ) {
	/**
	 ...
	 */
	function preservation_merge_js( $to, $list ) {
		$s = '';
		foreach ( $list as $f ) {
			$s .= preservation_fgc( preservation_get_file_dir( $f ) );
		}
		if ( '' != $s ) {
			$file_dir = preservation_get_file_dir( $to );
			if ( empty( $file_dir ) && strpos( $to, '-full.js' ) !== false ) {
				$file_dir = preservation_get_file_dir( str_replace( '-full.js', '.js', $to ) );
				if ( ! empty( $file_dir ) ) {
					$file_dir = str_replace( '.js', '-full.js', $file_dir );
				}
			}
			preservation_fpc( $file_dir,
				'/* '
				. strip_tags( __( "ATTENTION! This file was generated automatically! Don't change it!!!", 'preservation' ) )
				. "\n----------------------------------------------------------------------- */\n"
				. apply_filters( 'preservation_filter_js_output', apply_filters( 'preservation_filter_prepare_js', $s, true ), $to )
			);
		}
	}
}

if ( ! function_exists( 'preservation_merge_css' ) ) {
	/**
	 ...
	 */
	function preservation_merge_css( $to, $list, $need_responsive = false ) {
		if ( $need_responsive ) {
			$responsive = apply_filters( 'preservation_filter_responsive_sizes', preservation_storage_get( 'responsive' ) );
		}
		$sizes  = array();
		$output = '';
		foreach ( $list as $f ) {
			$fdir = preservation_get_file_dir( $f );
			if ( '' != $fdir ) {
				$css = preservation_fgc( $fdir );
				if ( $need_responsive ) {
					$pos = 0;
					while( false !== $pos ) {
						...
						if ( false !== $pos ) {
							...
							$sizes[ $media ] .= "\n\n" . apply_filters( 'preservation_filter_merge_css', substr( $css, $pos_lbrace + 1, $pos_rbrace - $pos_lbrace - 1 ) );
							$css = substr( $css, $pos_rbrace + 1);
						}
					}
				} else {
					$output .= "\n\n" . apply_filters( 'preservation_filter_merge_css', $css );
				}
			}
		}
		if ( $need_responsive ) {
			foreach ( $responsive as $k => $v ) {
				...
				if ( ! empty( $sizes[ $media ] ) ) {
					$output .= "\n\n"
							...
							. strip_tags( sprintf( __( '/* SASS Suffix: --%s */', 'preservation' ), $k ) )
							. "\n"
							. "@media {$media} {\n"
								. $sizes[ $media ]
							. "\n}\n";
			...
			if ( count( $sizes ) > 0 ) {
				$output .= "\n\n"
						. strip_tags( __( '/* Unknown Suffixes: */', 'preservation' ) );
		...
		if ( '' != $output ) {
			...
			if ( empty( $file_dir ) && strpos( $to, '-full.css' ) !== false ) {
				$file_dir = preservation_get_file_dir( str_replace( '-full.css', '.css', $to ) );
				if ( ! empty( $file_dir ) ) {
			...
			preservation_fpc( $file_dir,
				...
				. strip_tags( __("ATTENTION! This file was generated automatically! Don't change it!!!", 'preservation') ) 
				. "\n----------------------------------------------------------------------- */\n"
				. apply_filters( 'preservation_filter_css_output', apply_filters( 'preservation_filter_prepare_css', $output, true ), $to )
			);
		}
	}
}

if ( ! function_exists( 'preservation_filter_merge_css' ) ) {
	add_filter( 'preservation_filter_merge_css', 'preservation_filter_merge_css' );
	/**
	 ...
	 */
	function preservation_filter_merge_css( $css ) {
		return str_replace( '../../../../', '../../../', $css);
	}
}

if ( ! function_exists( 'preservation_merge_sass' ) ) {
	/**
	 ...
	 */
	function preservation_merge_sass( $to, $list, $need_responsive = false, $root = '../' ) {
		if ( $need_responsive ) {
			$responsive = apply_filters( 'preservation_filter_responsive_sizes', preservation_storage_get( 'responsive' ) );
		}
		$sass                = array(
		...
		foreach ( $list as $f ) {
			...
			if ( $add ) {
				$sass['import'] .= apply_filters( 'preservation_filter_sass_import', "{$sass_import} \"{$root}{$f}\";\n", $f );
				$save            = true;
			}
		}
		if ( $save ) {
			...
			preservation_fpc( preservation_get_file_dir( $to ), apply_filters( 'preservation_filter_sass_output', $output, $to ) );
		}
	}
}
...
if ( ! function_exists( 'preservation_get_domain_from_url' ) ) {
	/**
	 ...
	 */
	function preservation_get_domain_from_url( $url ) {
		$pos = strpos( $url, '//' );
		if ( $pos !== false ) {
...
if ( ! function_exists( 'preservation_add_protocol' ) ) {
	/**
	 ...
	 */
	function preservation_add_protocol( $url ) {
		return substr( $url, 0, 2 ) == '//' ? preservation_get_protocol() . ':' . $url : $url;
	}
}

if ( ! function_exists( 'preservation_remove_protocol_from_url' ) ) {
	/**
	 ...
	 * @return string         A processed string.
	 */
	function preservation_remove_protocol_from_url( $url, $complete = true ) {
		return preg_replace( '/(http[s]?:)?' . ( $complete ? '\\/\\/' : '' ) . '/', '', $url );
	}
}

if ( ! function_exists( 'preservation_add_to_url' ) ) {
	/**
	 ...
	 */
	function preservation_add_to_url( $url, $prm ) {
		if ( is_array( $prm ) && count( $prm ) > 0 ) {
			...
			$params = preservation_array_merge( $params, $prm );
			$url = $parts[0];
			$separator = '?';
...
if ( ! function_exists( 'preservation_is_url' ) ) {
	/**
	 ...
	 */
	function preservation_is_url( $url ) {
		return strpos( $url, '//' ) === 0 || strpos( $url, '://' ) !== false;
	}
}

if ( ! function_exists( 'preservation_is_external_url' ) ) {
	/**
	 ...
	 */
	function preservation_is_external_url( $url ) {
		return preservation_is_url( $url ) && strpos( $url, preservation_remove_protocol_from_url( get_home_url(), true ) ) === false;
	}
}

if ( ! function_exists( 'preservation_url_to_local_path' ) ) {
	/**
	 ...
	 */
	function preservation_url_to_local_path( $url ) {
		$path = '';
		...
		$locations = array(
			'uploads' => array(
				...
				'url' => preservation_remove_protocol_from_url( $upload_info['baseurl'] )
				),
			'child' => array(
				'dir' => PRESERVATION_CHILD_DIR,
				'url' => preservation_remove_protocol_from_url( PRESERVATION_CHILD_URL )
				),
			'theme' => array(
				'dir' => PRESERVATION_THEME_DIR,
				'url' => preservation_remove_protocol_from_url( PRESERVATION_THEME_URL )
				)
			);
