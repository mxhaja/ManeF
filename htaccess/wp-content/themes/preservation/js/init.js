...
jQuery( document ).ready( function() {
	...
	function preservation_init_media_elements(cont) {
		var audio_selector = preservation_apply_filters( 'preservation_filter_mediaelements_audio_selector', 'audio:not(.inited)' ),
	...
	function preservation_tabs_ajax_content_loader(panel, page, oldPanel) {
		...
		jQuery.post(
			...
			if (rez.error !== '') {
				...
				// Append posts
				panel
					.find( '.preservation_tab_holder,.preservation_tab_content_remove' ).remove().end()
	...
	function preservation_resize_video(cont) {
		...
		if ( PRESERVATION_STORAGE['resize_tag_iframe'] ) {
			cont.find( '.video_frame iframe,iframe' ).each( function() {
				var $self = jQuery( this );
				// If item now invisible
				if ( $self.hasClass( 'trx_addons_resize' ) || $self.hasClass( 'trx_addons_noresize' ) || $self.addClass( 'preservation_resize' ).parents( 'div:hidden,section:hidden,article:hidden' ).length > 0 ) {
					return;
				}
