...
jQuery( document ).ready(
	function() {
		...
		preservation_add_filter('trx_addons_filter_sc_classes', function(classes, settings) {
		...
		setTimeout( function() {
			if ( window.elementor !== undefined ) {
				...
				jQuery( '#elementor-panel' )
					.on( 'input change', '[data-setting^="preservation_options_"]', function (e) {
						...
						var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.preservation_post_editor' );
						if ( link.length > 0 ) {
							preservation_change_post_edit_link_elementor( link );
						}
					} )
					.on( 'click', '.preservation_post_editor', function(e) {
						preservation_change_post_edit_link_elementor( jQuery(this) );
						if (jQuery(this).hasClass('preservation_hidden' )) {
							e.preventDefault();
							return false;
						}
					});
		...
		function preservation_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
