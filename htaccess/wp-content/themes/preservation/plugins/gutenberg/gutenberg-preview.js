...
jQuery( window ).on( 'load', function() {
	...
	if ( $editor_wrapper.length ) {
		...
		if ( typeof window.MutationObserver !== 'undefined' ) {
			...
			preservation_create_observer( 'preservation-check-visual-editor-wrapper', $editor_wrapper, function( mutationsList ) {
				preservation_gutenberg_editor_init();
			} );
		} else {
		 	preservation_gutenberg_editor_init();
		}
	...
	function preservation_gutenberg_editor_object() {
		var editor = {
			$editor: false,
			$frame: false,
			...
			if ( $editor.length ) {
				...
				if ( editor_type == 'site' ) {
					...
					if ( editor.$frame.length && editor.$frame.get(0).contentDocument ) {
						...
						if ( editor.$styles_wrapper.hasClass( 'preservation_inited' ) ) {
					...
					if ( ! editor.$editor.hasClass( 'preservation_inited' ) ) {
	...
	// Init on page load
	function preservation_gutenberg_editor_init() {

		// Get Gutenberg editor object
		var editor = preservation_gutenberg_editor_object();
		if ( ! editor.$editor ) {
			return;
		}
		...
		function add_class_with_color_scheme( $obj ) {
			if ( ! $obj.hasClass( 'scheme_' + PRESERVATION_STORAGE['color_scheme'] ) ) {
				$obj.addClass( 'scheme_' + PRESERVATION_STORAGE['color_scheme'] );
			}
		}

		function add_class_with_overridden_options( $obj ) {
			for ( var i in PRESERVATION_STORAGE['override_classes'] ) {
				$obj.addClass( PRESERVATION_STORAGE['override_classes'][i].replace( '%s', PRESERVATION_STORAGE[ i ] ) );
			}
		}
		...
		if ( editor_type == 'post' && editor.$styles_wrapper ) {
			...
			editor.$styles_wrapper.addClass( preservation_get_class_by_prefix( editor.$editor.attr( 'class' ), 'post-type-' ) );
			...
			if ( editor.$writing_flow && PRESERVATION_STORAGE['sidebar_position'] != 'hide' ) {
		...
		if ( editor_type == 'site' && editor.$styles_wrapper ) {
			...
			add_class_with_overridden_options( editor.$styles_wrapper );
			// Mark as inited
			editor.$styles_wrapper.addClass( 'preservation_inited' );
		}
		...
		if ( editor_type == 'widgets' && editor.$writing_flow ) {
			...
			preservation_remove_observer( 'preservation-check-editor-styles-wrapper' );
			preservation_create_observer( 'preservation-check-editor-styles-wrapper', editor.$writing_flow, function( mutationsList ) {
		...
		editor.$editor.addClass( 'preservation_inited' );
	}
} );
