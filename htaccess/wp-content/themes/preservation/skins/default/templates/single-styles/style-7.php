...
if ( apply_filters( 'preservation_filter_single_post_header', is_singular( 'post' ) || is_singular( 'attachment' ) ) ) {
	...
	ob_end_clean();
	$preservation_with_featured_image = preservation_is_with_featured_image( $preservation_post_header, array( 'with_gallery' ) );
	// Post title and meta
	ob_start();
	...
	if ( strpos( $preservation_post_header, 'post_featured' ) !== false
		...
		<div class="post_header_wrap post_header_wrap_in_header post_header_wrap_style_<?php
            ...
            if ( $preservation_with_featured_image ) {
                echo ' with_featured_image' . ( false && preservation_get_theme_option( 'single_parallax' ) == 0 ? ' preservation-full-height' : '' );
            }
		?>">
			<?php
