	...
	foreach( $template_parts as $k=>$v ) {
		...
		if ( substr( $k, 0, 4 ) == 'meta') {
			...
			if ($args['template_list'] == 'hover') {
				$image = get_the_post_thumbnail_url(get_the_ID(), preservation_get_thumb_size('big'));
			}
			...
			if (false && $args['type'] == 'list' && $args['template_list'] == 'hover_2') {
				...
				if(!empty($title)) {
					...
					echo preservation_strwords($title, 11);
					preservation_show_layout((!empty($post_link) ? '</a>' : '') . '</h6>');
				}
			} else {
