/* global preservation_color_schemes, preservation_dependencies, Color */
...
( function( api ) {
	...
	for (var i in preservation_color_schemes) {
		cssTemplate['scheme_' + i] = wp.template( 'preservation-color-scheme-' + i );
	}
	// Add Template with theme fonts
	cssTemplate['theme_fonts'] = wp.template( 'preservation-fonts' );
	// Add Template with theme vars
	cssTemplate['theme_vars'] = wp.template( 'preservation-vars' );
	...
	api.bind( 
		'ready', function() {
			...
			jQuery( '#customize-header-actions #save' )
				.before( '<input type="button" class="button customize-action-reset" value="' + preservation_customizer_vars['msg_reset'] + '">' );
			jQuery( '#customize-header-actions .customize-action-reset' )
				.on( 'click', function(e) {
					if (confirm( preservation_customizer_vars['msg_reset_confirm'] )) {
			...
			jQuery( '#customize-header-actions .spinner' )
				.after( '<button class="button customize-action-refresh icon-spin3" title="' + preservation_customizer_vars['msg_reload_title'] + '">' + preservation_customizer_vars['msg_reload'] + '</button>' );
			...
			if (preservation_customizer_vars && preservation_customizer_vars['theme_name_suffix']) {
				jQuery( '#customize-info .site-title' ).append( preservation_customizer_vars['theme_name_suffix'] );
			}
			...
			jQuery( '#customize-theme-controls [id^="customize-control-load_fonts"]' )
				.on( 'change', preservation_customizer_update_load_fonts );
			...
			jQuery( '#customize-theme-controls .control-section .customize-control-button input[type="button"]' )
				.on( 'click', preservation_customizer_actions );

			// Check dependencies in the each section on ready
			jQuery( '#customize-theme-controls .control-section' )
				.each( function () {
					preservation_customizer_check_dependencies( jQuery( this ) );
				} );
			...
			jQuery( '#customize-theme-controls .control-section > .accordion-section-title' )
				.on( 'click', function() {
					...
					if (id !== '') {
						...
						if (section.length > 0) {
							preservation_customizer_check_dependencies( section );
						}
					}
				} );

			// Decorate subsections in the panel 'Typography'
			if ( preservation_customizer_vars['decorate_fonts'] ) {

				jQuery('body').addClass( 'preservation_decorate_fonts_section' );

				// Prepare section for open on title is clicked
				var preservation_customizer_title_clicked = function( title ) {
					var id = title.parent().attr( 'aria-owns' );
					preservation_customizer_close_opened_sections( title.parents( '#sub-accordion-panel-fonts' ), id );
					var section = jQuery( '#' + id ),
					...
					.each ( function() {
						...
						$self
							.on( 'keydown', function(e) {
								if ( e.which == 13 ) {
									preservation_customizer_title_clicked( $self );
								}
							} )
							.on( 'click', function() {
								preservation_customizer_title_clicked( $self );
				...
				var preservation_customizer_close_opened_sections = function( panel, current_id ) {
					if ( panel.length ) {
				...
				jQuery( '#customize-theme-controls #sub-accordion-panel-fonts .customize-panel-back' )
					.on( 'keydown', function(e) {
						if ( e.which == 13 ) {
							preservation_customizer_close_opened_sections( jQuery(this).parents('#sub-accordion-panel-fonts') );
						}
					} )
					.on( 'click', function() {
						preservation_customizer_close_opened_sections( jQuery(this).parents('#sub-accordion-panel-fonts') );
	...
	api.bind( 'change', function(obj) {
		...
		preservation_customizer_check_dependencies( jQuery( '#customize-theme-controls #customize-control-' + obj.id ).parents( '.control-section' ) );
		preservation_customizer_refresh_preview( obj );
	} );
	...
	api.bind( 'refresh_schemes', function() {
		...
		for (i in cssTemplate) {
			...
			i = i.replace( 'scheme_', '' );
			if (typeof preservation_color_schemes[i] === 'undefined') {
				delete cssTemplate['scheme_' + i];
				jQuery( '#tmpl-preservation-color-scheme-' + i ).remove();
			} else if (tpl_idx == '') {
				tpl_idx     = i;
				tpl_content = jQuery( '#tmpl-preservation-color-scheme-' + i ).html().trim();
			}
		}
		// Add new templates
		var regex = new RegExp( "\.scheme_" + tpl_idx, "g" );
		for (i in preservation_color_schemes) {
			if (typeof cssTemplate['scheme_' + i] === 'undefined') {
				jQuery( '#tmpl-preservation-color-scheme-' + tpl_idx )
				.clone()
				.attr( 'id', 'tmpl-preservation-color-scheme-' + i )
				.html( tpl_content.replace( regex, '.scheme_' + i ) )
				.insertAfter( jQuery( '#tmpl-preservation-color-scheme-' + tpl_idx ) );
				cssTemplate['scheme_' + i] = wp.template( 'preservation-color-scheme-' + i );
			}
		}
	} );
	...
	for (var action in preservation_customizer_vars['actions']['expand']) {
		if (action == 'length') {
			continue;
		}
		if (preservation_customizer_vars['actions']['expand'][action]['type'] == 'panel') {
			api.panel(
				action, function( panel ) {
					panel.expanded.bind(
						function( isExpanded ) {
							if ( isExpanded ) {
								var data = preservation_customizer_vars['actions']['expand'][panel.id];
			...
			api.section(
				action, function( section ) {
					section.expanded.bind(
						function( isExpanded ) {
							if ( isExpanded ) {
								var data = preservation_customizer_vars['actions']['expand'][section.id];
								if (typeof data['url'] !== 'undefined' && data['url'] !== '') {
									api.previewer.previewUrl.set( data['url'] );
								}
								if (typeof data['callback'] !== 'undefined' && data['callback'] !== '' && typeof window[data['callback']] === 'function') {
	...
	function preservation_customizer_get_field_value(fld) {
	...
	function preservation_customizer_check_dependencies(cont) {
		if ( typeof preservation_dependencies == 'undefined' || PRESERVATION_STORAGE['check_dependencies_now'] ) return;
			...
			function() {
				...
				if (depend) {
					...
					for (i in depend) {
						...
						if (fld.length > 0) {
							val = preservation_customizer_get_field_value( fld );
							if ( val == 'inherit' ) {
								dep_cnt = 0;
								dep_all = 1;
								var tag;
	...
	function preservation_customizer_refresh_preview(obj) {
		...
		if (updateCSS) {
			...
			if (id == 'scheme_storage') {
				...
				// Any color in the scheme_storage is changed
			} else if (Object.values( preservation_sorted_schemes ).indexOf( id ) >= 0) {
				forceUpdateCSS = true;
				...
				if (var_name !== undefined) {
					...
					preservation_customizer_update_theme_vars( jQuery( '[data-customize-setting-link="' + id + '"]' ).data( 'var_name' ), val );
					processed = true;
				}

				// Any theme fonts parameter is changed
			} else {
				for (opt in preservation_theme_fonts) {
					for (rule in preservation_theme_fonts[opt]) {
						if (opt + '_' + rule == id) {
							// Store new value to the fonts table
							preservation_customizer_update_theme_fonts( opt, rule, val );
							processed = true;
							break;
						}
					}
			...
			if (processed || forceUpdateCSS) {
				preservation_customizer_update_css();
			}
		}
	...
	// Actions buttons
	function preservation_customizer_actions(e) {
	...
	function preservation_customizer_update_theme_vars(opt, value) {
		preservation_theme_vars[opt] = parseFloat( value );
	}

	// Store new value in the theme fonts
	function preservation_customizer_update_theme_fonts(opt, rule, value) {
		preservation_theme_fonts[opt][rule] = value;
	}

	// Change theme fonts options if load fonts is changed
	function preservation_customizer_update_load_fonts() {
		var opt_list = [], i, tag, sel, opt, name = '', family = '', val = '', new_val = '', sel_idx = 0;
		updateCSS    = false;
		for (i = 1; i <= preservation_customizer_vars['max_load_fonts']; i++) {
	...
	function preservation_customizer_update_css() {
		...
		var vars = preservation_customizer_add_theme_vars( preservation_theme_vars );

		// Make styles and add its to the css
		css += preservation_customizer_prepare_html_value( cssTemplate['theme_vars']( vars ) );

		// Add theme specific fonts rules
		var fonts = preservation_customizer_add_theme_fonts( preservation_theme_fonts );

		// Make styles and add its to the css
		css += preservation_customizer_prepare_html_value( cssTemplate['theme_fonts']( fonts ) );

		// Add colors
		var sorted = {}, scheme = '';
		for (var param in preservation_sorted_schemes) {
			if (param == 'length') {
				continue;
			}
			scheme = api( preservation_sorted_schemes[param] )();
			if (scheme && scheme !== 'inherit') {
				sorted[scheme] = 1;
			}
		}
		for (scheme in preservation_color_schemes) {
			sorted[scheme] = 1;
		}
		for (scheme in sorted) {
			var i, colors = [];
			// Copy all colors to the temp var
			for (i in preservation_color_schemes[scheme].colors) {
				colors[i] = preservation_color_schemes[scheme].colors[i];
			}
			// Add theme specific colors and tints
			colors = preservation_customizer_add_theme_colors( colors );
			...
			var tmpl = jQuery( '#tmpl-preservation-color-scheme-' + scheme ).html().trim();
			for (i in colors) {
	...
	function preservation_customizer_add_theme_colors(colors) {
		if (preservation_additional_colors) {
			var clr = '', v = '';
			for (var k in preservation_additional_colors) {
				v   = preservation_additional_colors[k];
				clr = colors[v['color']];
		...
		for (var tag in fonts) {
			...
			rez[tag + '_font-size']       = typeof fonts[tag]['font-size'] != 'undefined'
									&& fonts[tag]['font-size'] != 'inherit'
												? 'font-size:' + preservation_customizer_prepare_css_value( fonts[tag]['font-size'] ) + ";"
												: '';
			...
			rez[tag + '_margin-top']      = typeof fonts[tag]['margin-top'] != 'undefined'
									...
									&& fonts[tag]['margin-top'] != 'inherit'
												? 'margin-top:' + preservation_customizer_prepare_css_value( fonts[tag]['margin-top'] ) + ";"
												: '';
			rez[tag + '_margin-bottom']   = typeof fonts[tag]['margin-bottom'] != 'undefined'
									&& fonts[tag]['margin-bottom'] !== ''
									&& fonts[tag]['margin-bottom'] != 'inherit'
												? 'margin-bottom:' + preservation_customizer_prepare_css_value( fonts[tag]['margin-bottom'] ) + ";"
												: '';
		}
		return rez;
	}
	...
	function preservation_customizer_add_theme_vars(vars) {
		...
		if ( typeof vars['rad'] != 'undefined' ) {
			...
			rez['rad']      = preservation_customizer_prepare_css_value( vars['rad'] );
		...
		if ( typeof vars['page_width'] != 'undefined' ) {
			...
			if ( isNaN( vars['page_width'] ) || vars['page_width'] === 0) {
				vars['page_width'] = preservation_customizer_vars['page_width_default'];
			}
			vars['sidebar_width']      = parseInt( vars['sidebar_width'], 10 );
			vars['sidebar_gap']        = parseInt( vars['sidebar_gap'], 10 );

			rez['page_width']          = preservation_customizer_prepare_css_value( vars['page_width'] );
			rez['page_boxed_extra']    = preservation_customizer_prepare_css_value( vars['page_boxed_extra'] );
			rez['page_fullwide_extra'] = preservation_customizer_prepare_css_value( vars['page_fullwide_extra'] );
			rez['page_fullwide_max']   = preservation_customizer_prepare_css_value( vars['page_fullwide_max'] );
			rez['grid_gap']            = preservation_customizer_prepare_css_value( vars['grid_gap'] );
			rez['sidebar_prc']         = vars['sidebar_width'] / vars['page_width'];
			rez['sidebar_gap_prc']     = vars['sidebar_gap'] / vars['page_width'];
			rez['sidebar_width']       = preservation_customizer_prepare_css_value( vars['sidebar_width'] );
			rez['sidebar_gap']         = preservation_customizer_prepare_css_value( vars['sidebar_gap'] );
			rez['sidebar_gap_width']   = preservation_customizer_prepare_css_value( vars['sidebar_gap'] );
		}
		return rez;
	}

	// Add ed to css value
	function preservation_customizer_prepare_css_value(val) {
		if (val !== '' && val != 'inherit') {
			var ed = ('' + val).substr( -1 );
			if ('0' <= ed && ed <= '9') {
				val += 'px';
	...
	function preservation_customizer_prepare_html_value(val) {
