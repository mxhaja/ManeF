 ...
 * @since PRESERVATION 1.0.22
 */

// If this theme is a free version of premium theme
if ( ! defined( 'PRESERVATION_THEME_FREE' ) ) {
	define( 'PRESERVATION_THEME_FREE', false );
}
if ( ! defined( 'PRESERVATION_THEME_FREE_WP' ) ) {
	define( 'PRESERVATION_THEME_FREE_WP', false );
}

// If this theme support external updates
if ( ! defined( 'PRESERVATION_THEME_ALLOW_UPDATE' ) ) {
	define( 'PRESERVATION_THEME_ALLOW_UPDATE', true );
}

// If this theme uses skins
if ( ! defined( 'PRESERVATION_ALLOW_SKINS' ) ) {
	define( 'PRESERVATION_ALLOW_SKINS', true );
}
if ( ! defined( 'PRESERVATION_DEFAULT_SKIN' ) ) {
	define( 'PRESERVATION_DEFAULT_SKIN', 'default' );
}
if ( ! defined( 'PRESERVATION_REMEMBER_SKIN' ) ) {
	define( 'PRESERVATION_REMEMBER_SKIN', false );
}
...
//-------------------------------------------------------------------------
$GLOBALS['PRESERVATION_STORAGE'] = array(
...
if ( ! function_exists( 'preservation_get_theme_pro_key' ) ) {
	/**
	 ...
	 */
	function preservation_get_theme_pro_key() {
		$theme_pro_key = get_option( sprintf( 'purchase_code_src_%s', get_template() ) );
		if ( empty( $theme_pro_key ) ) {
			$theme_pro_key = preservation_storage_get( 'theme_pro_key' );
		}
		return $theme_pro_key;
	}
}

if ( ! function_exists( 'preservation_trx_updater_get_theme_info' ) ) {
	add_filter( 'trx_updater_filter_get_theme_info', 'preservation_trx_updater_get_theme_info' );
	/**
	 * Add an information about the theme engine to the array with a theme info.
	 *
	 * Hooks: add_filter( 'trx_updater_filter_get_theme_info', 'preservation_trx_updater_get_theme_info' );
	 *
	 * @param array $info  An array with a theme information.
	 *
	 * @return array       A theme information with an engine info.
	 */
	function preservation_trx_updater_get_theme_info( $info ) {
		$engine = preservation_storage_get( 'engine' );
...
if ( ! function_exists( 'preservation_original_theme_slug' ) ) {
	add_filter( 'preservation_filter_original_theme_slug', 'preservation_original_theme_slug' );
	/**
	 ...
	 *
	 * Hooks: add_filter( 'preservation_filter_original_theme_slug', 'preservation_original_theme_slug' );
	 *
	 * @param string $theme_slug  Current slug of the theme.
	 *
	 * @return string             Original slug of the theme.
	 */
	function preservation_original_theme_slug( $theme_slug = '' ) {
		return 'preservation';
	}
}
...
if ( ! function_exists( 'preservation_importer_set_options' ) ) {
	add_filter( 'trx_addons_filter_importer_options', 'preservation_importer_set_options', 9 );
	/**
	 * Set a theme-specific options for the import of the demo data via plugin ThemeREX Addons.
	 *
	 * Hooks: add_filter( 'trx_addons_filter_importer_options', 'preservation_importer_set_options', 9 );
	 *
	 * @param array $options  Options for the import of the demo data.
	 *
	 * @return array          Processed options with a theme-specific values.
	 */
	function preservation_importer_set_options( $options = array() ) {
		if ( is_array( $options ) ) {
			...
			// Prepare demo data
			$options['demo_url'] = esc_url( preservation_get_protocol() . ':' . preservation_storage_get( 'theme_demofiles_url' ) );
			// Required plugins
			$options['required_plugins'] = array_keys( preservation_storage_get( 'required_plugins' ) );
...
if ( ! function_exists( 'preservation_ocdi_set_options' ) ) {
	add_filter( 'trx_addons_filter_ocdi_options', 'preservation_ocdi_set_options', 9 );
	/**
	 ...
	 * @return array          Processed options with a theme-specific values.
	 */
	function preservation_ocdi_set_options( $options = array() ) {
		if ( is_array( $options ) ) {
			// Prepare demo data
			$options['demo_url'] = esc_url( preservation_get_protocol() . ':' . preservation_storage_get( 'theme_demofiles_url' ) );
			// Required plugins
...
if ( ! function_exists( 'preservation_customizer_theme_setup1' ) ) {
	add_action( 'after_setup_theme', 'preservation_customizer_theme_setup1', 1 );
	/**
	 * Create an internal setting for the theme.
	 * Attention! This settings ONLY FOR PROGRAMMERS, NOT FOR CUSTOMER
	 *
	 * Hooks: add_action( 'after_setup_theme', 'preservation_customizer_theme_setup1', 1 );
	 *
	 ...
	 */
	function preservation_customizer_theme_setup1() {
		preservation_storage_set(
			'settings', array(
				...
				'allow_fullscreen'          => false,                   // Allow cases 'fullscreen' and 'fullwide' for the body style in the Theme Options
																		...
																		// (can be removed if filter 'preservation_filter_allow_fullscreen' return false)
